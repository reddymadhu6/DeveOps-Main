...
$showForm = true; // Variable to track if the form should be displayed
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($conn->connect_error) {
        ...
        if ($execval) {
            ...
            $_SESSION['booking_success'] = true; // Set the success state in session variable
...
if (isset($_SESSION['booking_success']) && $_SESSION['booking_success']) {
    $showForm = false; // Hide the form
    unset($_SESSION['booking_success']); // Clear the success state from session
}
?>
...
<?php else: ?>
      <!-- Success message -->
      <div class="mt-4">
        <p class="text-success" style="font-size: xx-large;">Booked Successfully!</p>
      </div>
    <?php endif; ?>
